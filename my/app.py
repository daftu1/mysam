...
def load_sam2_model():
    # 初始化SAM2视频预测器
    checkpoint = "sam2.1_hiera_base_plus.pt"  # 替换为您的模型路径
            ...
            if not ref_points or not label:
                ...
                with torch.autocast("cuda" if torch.cuda.is_available() else "cpu"):
                    ...
                    for i in range(1, len(frame_files)):
                        frame_idx, obj_ids, masks = sam2_model.propagate_to_next_frame(
                            inference_state=inference_state,
                            frame_idx=i-1
                        )
                        ...
                        h, w = mask.shape
                        yolo_line = f"{label_id} {(x1+x2)/2/w:.6f} {(y1+y2)/2/h:.6f} {(x2-x1)/w:.6f} {(y2-y1)/h:.6f}\n"
                        label_file = os.path.join(save_dir, frame_files[i].replace(".jpg", ".txt"))
                        with open(label_file, "w") as f:
                            f.write(yolo_line)
